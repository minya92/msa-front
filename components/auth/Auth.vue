  ...
  <modal-fade @close="closeForm">
    <div class="modal__auth__content">
      ...
      <div class="form-group">
        ...
        <span v-if="email.showError" class="text-error">{{
          email.textError
        }}</span>
      </div>
      <div class="form-group">
        ...
        <span v-if="password.showError" class="text-error">{{
          password.textError
        }}</span>
      </div>
      ...
      <div class="alt-auth">
        <SocialAuth />
        <div>
          <router-link :to="{ name: 'RestorePassword' }" @click="forgotPassword"
            >Забыли пароль?
          </router-link>
        </div>
      </div>
...
export default {
  ...
  methods: {
    submitForm: function() {
      doAuth(this.email.value, this.password.value)
        .then(r => {
          this.$router.go({
