<template>
  <div class="recent-view" v-if="products.length > 0">
    ...
    <div class="prev-next">
      <div
        :class="['prev-result', PrevIsActive() ? '' : 'disabled']"
        @click="prevResult"
      >
        &lt;
      </div>
      <div
        :class="['next-result', NextIsActive() ? '' : 'disabled']"
        @click="nextResult"
      >
        &gt;
      </div>
...
export default {
      ...
      blockActions: [
        {
          ...
          footerText:
            'В наличии и под заказ любые комплектующие для вашего мотоцикла. Для постоянных покупателей действуют скидки и промо-коды',
  ...
  methods: {
    NextIsActive: function() {
      return (
        this.paramNextPrev.perItems * this.paramNextPrev.indexResultItems <
        this.paramNextPrev.totalProducts
      );
    },
    ...
    nextResult: function() {
      if (this.NextIsActive()) {
        this.paramNextPrev.currentItems =
          this.paramNextPrev.perItems * this.paramNextPrev.indexResultItems;
    ...
    showMore: function(index) {
      return (
        index >= this.paramNextPrev.currentItems &&
        index <
          this.paramNextPrev.perItems * this.paramNextPrev.indexResultItems
      );
    }
...
.prev-result:not(.disabled):hover,
.next-result:not(.disabled):hover {
