    ...
    >
        ...
        <tbody>
          ...
          <tr v-for="(product, index) in products" :key="index">
            <td class="table__cart__picture">
              <router-link :to="'product/' + product.cost_id"
                ><img :src="product.image"
              /></router-link>
            </td>
            <td class="table__cart__description">
              <router-link
                class="table__cart__title-item"
                :to="'product/' + product.cost_id"
              >
                {{ product.name }}
              </router-link>
            ...
            <td class="table__cart__quantity">
              <button class="quantity_minus" @click="quantityMinus(product)">
                -
              </button>
              <input :value="product.quantity" />
              <button class="quantity_plus" @click="quantityPlus(product)">
                +
              </button>
            </td>
            <td class="table__cart__price">{{ product.price }}</td>
            <td class="table__cart__remove">
              <div class="table__cart__remove_btn" @click="removeItem(product)">
                x
              </div>
            </td>
          </tr>
      ...
      <div class="cart-methods-section">
        ...
        <div class="simplecheckout-right-column">
          <payment-methods v-model="selectPayment" :total="total" />
          <shipping-methods v-model="selectDelivery" :total="total" />
        </div>
      </div>
      ...
      <div class="cart__footer">
        <button class="btn_theme_white btn_cart" @click="goCatalog">
          Продолжить покупки
        </button>
        <button class="btn_theme btn_cart" @click="sendOrder">
          Оформить заказ
        </button>
        <div>
...
export default {
  ...
  computed: {
    ...
    selectDeliveryMethods() {
      ...
      const deliverCost = this.selectDelivery.deliverCost
        ? this.selectDelivery.deliverCost
        : 0;
  ...
  methods: {
    ...
    sendOrder: function() {
      ...
      let newJson = [];
      this.$store.getters.cartProducts.forEach(x =>
        newJson.push({ itemsCount: x.quantity, cost_id: x.id })
      );

      let post =
        `fio=${this.userFields.user}&phone=${this.userFields.phone}` +
    ...
    this.$API
      ...
      .then(r => {
        ...
        r.data.data.forEach(item => {
          ...
          let tempQuantity = this.$store.getters.cartProducts.filter(x => {
            if (item.cost_id == x.id) {
              return x.quantity;
            }
          });
          ...
          if (tempQuantity.length > 0) {
            this.products.push({
              ...
              quantity: this.$store.getters.cartProducts.filter(x => {
                if (item.cost_id == x.id) {
                  return x.quantity;
                }
              })[0].quantity
