    ...
    <template v-if="product">
        ...
        <div class="product__detail">
          <div class="product__detail__media">
            <div class="main-image">
              ...
              <p class="main-image__tooltip">
                Кликните для просмотра в полный экран
              </p>
            </div>
            <div class="thumblist">
              <a v-for="(image, index) in product.images" :key="image.id"
                ><img
                  :src="loadImage(image.thumbnail)"
                  @click="numCurrentImage = index"
              /></a>
            </div>
          </div>
          <div class="product__detail__description">
            ...
            <div class="article" v-if="product.article">
              артикул: {{ product.article }}
            </div>
            <button @click="addToCart(product)">
              <img src="../../assets/img/cart_empty.svg" />Вкорзину
            </button>
            <button @click="oneClickShow = true">
              <img src="../../assets/img/cart_empty.svg" />Купить в 1 клик
            </button>
            <template v-if="product.chars">
              <div class="characteristic-block">
                <h3>Характеристики</h3>
                <div v-for="(char, index) in product.chars" :key="index">
                  {{ char.char_name }}: <strong>{{ char.char_value }}</strong>
                </div>
              </div>
            </template>
            <div>
              <div>
                На фотографии изображен только вариант расцветки пластика.
                Комплектность поставки полностью совпадает с оригинальным
                комплектом, поставляемым заводом изготовителем мотоцикла для
                этой модели
              </div>
            </div>
          </div>
      ...
      <template v-if="product.compatibility">
        <div class="content-fluid product-compatibility">
          ...
          <ul>
            <li
              v-for="(compatibility, index) in product.compatibility"
              :key="index"
            >
...
export default {
  ...
  computed: {
    ...
    loadFullImage: function() {
      ...
      return (
        ...
        this.product.images[this.numCurrentImage].full_image
      );
    }
  },
  methods: {
    ...
    selectImage: function(elem, index) {
      let imageSrc =
        this.$store.state.baseUrl + this.product.images[index].full_image;
  ...
  async asyncData({ store, route }) {
    ...
    try {
      let item = await apiCatalog.getItem(route.params.id);
      product = {
        id: item.cost_id,
        name: item.item_name,
      ...
      full_images = item.images.map(x => store.state.baseUrl + x.full_image);
      this.$store.dispatch('recentItem', this.product);
    } catch (exp) {}

    return { product, full_images };
  }
};
</script>
...
.product__detail_description,
.product-compatibility {
  display: flex;
  flex-direction: column;
}
.product__detail_description h3,
